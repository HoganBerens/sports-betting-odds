...
const Dashboard = () => {
  ...
  const getOdds = () => {
    ...
    /* axios
      .get(
        `https://api.the-odds-api.com/v4/sports/icehockey_nhl/odds/?apiKey=${API_KEY}&regions=us&markets=h2h,spreads&oddsFormat=american&bookmakers=draftkings&commenceTimeFrom=${
          start.toISOString().split('.')[0] + 'Z'
        }&commenceTimeTo=${end.toISOString().split('.')[0] + 'Z'}`
      )
      .then((response) => {
